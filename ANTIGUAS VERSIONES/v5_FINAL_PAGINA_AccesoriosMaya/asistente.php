<?php 
    ...
    include_once 'asistente/preguntas3.php'; // El nuevo archivo de humor
...
<div id="chat-window">
    <div style="background: #2c3e50; color: white; padding: 18px; display: flex; align-items: center; justify-content: space-between;">
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
                ...
                <small style="color: #2ecc71; font-size: 0.75rem;">● Experto en Tesoros para Mascotas</small>
            </div>
        </div>
    ...
    <div id="chat-body" style="flex: 1; padding: 20px; overflow-y: auto; background: #fdfdfd; display: flex; flex-direction: column; gap: 15px;">
        <div class="bubble-bot">
            ¡Hola! Soy <b>Dankiel</b>. 🦝 He estado husmeando por el almacén y encontré cosas increíbles. ¿Qué tesoro buscamos hoy, algo para un Perro o un Gato?
        </div>
    </div>
...
function enviarMensajeDankiel() {
    ...
    // Simulamos que Dankiel está pensando
    setTimeout(() => {
        let resp = "";

        // 1. Lógica de Catálogo
        if (msg.includes("ayuda") || msg.includes("disponible") || msg.includes("catalogo") || msg.includes("productos")) {
            addBubble("¡Entendido! Dankiel ha seleccionado estas joyas de nuestro inventario:", 'bot');
        ...
        // 2. Lógica de Navegación (Confirmaciones)
        else if (msg === "si" || msg === "claro" || msg.includes("por favor")) {
            if (contextoMascota === "perros") {
                addBubble("¡Guau! Excelente elección. ¡Dankiel te guía al paraíso canino!", 'bot');
        ...
        // 3. Detección de Secciones
            ...
            resp = "¡Los perros aman nuestros accesorios! 🐕 ¿Te gustaría que te lleve a ver los productos para perros?";
            contextoMascota = "perros";
        }
        else if (msg.includes("gato")) {
            resp = "¡Miau! Tengo rascadores y juguetes que les fascinan. 🐈 ¿Quieres ver la sección de gatos?";
            contextoMascota = "gatos";
        }
        // 4. Búsqueda en Diccionarios (Ventas + Humor)
        else {
            ...
            if (!encontrado) resp = "Dankiel se quedó pensando... 🤔 No encontré eso en mi mapa de tesoros, pero pregúntame por secciones o productos.";
        }
...
// Escuchar la tecla Enter para enviar
